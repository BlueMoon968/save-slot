...
const addGame = () => {
  ...
  const exportCSV = () => {
    const isWishlistExport = activeTab === 'wishlist';
    const gamesToExport = isWishlistExport ? wishlist : games;
    const exportType = isWishlistExport ? 'wishlist' : 'collection';
    const csvContent = [
    ...
    link.download = `save-slot-${exportType}-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  const filteredGames = useMemo(() => {
    return games.filter(game => {
      ...
      if (sortOrder === 'console') {
        return a.console.localeCompare(b.console, 'it', { sensitivity: 'base' }) ||
          a.title.localeCompare(b.title, 'it', { sensitivity: 'base', numeric: true });
      }
      return a.title.localeCompare(b.title, 'it', { sensitivity: 'base', numeric: true });
    });
  }, [games, searchTerm, filterConsole, filterVersion, sortOrder]);
  ...
  }, [wishlist, searchTerm, filterConsole, filterVersion, sortOrder]);
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4 sm:mb-6">
            ...
            <div className="flex flex-wrap gap-2 justify-center sm:justify-end">
              ...
              <button
                onClick={exportCSV}
                className="px-3 py-2 sm:px-4 sm:py-2 bg-slate-700 text-white rounded-sm hover:bg-slate-600 transition-all font-bold border-4 border-slate-600 hover:border-slate-500 flex items-center gap-2 font-mono text-xs sm:text-sm"
                title={`Export ${activeTab === 'wishlist' ? 'wishlist' : 'collection'} as CSV`}
              >
            ...
            <div className="mb-4 sm:mb-6 bg-slate-800 p-3 sm:p-4 rounded-sm border-4 border-slate-700">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
                ...
                >
                  ...
                  <option value="console">CONSOLE A-Z</option>
                </select>
              </div>
